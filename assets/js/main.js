...
// function httpGet(value, theUrl="https://api.telegram.org/bot1312453521:AAFrfJS1ikwGIN3KkuDWVDB0n8NHcM4eRUU/sendMessage?chat_id=-3213121&text=") { 	
...
window.onload = function() {
  ...
  form1.onsubmit = submittedFormOne.bind(form1);

  var form2 = document.querySelector("#contact_form_2");
...
function submittedFormOne(event) {
  ...
  for (let i = 0; i < inputs.length; i++) {
    let fieldName = inputs[i].id.replace('_contact_form_1', '').replace('_', " ").toUpperCase()
...
function submittedFormTwo(event) {
  ...
  let message = 'Message from website Octopus Group:%0A'
  for (let i = 0; i < inputs.length; i++) {
    let fieldName = inputs[i].id.replace('_contact_form_2', '').replace('_', " ").toUpperCase()
    let extraValueData = " ".repeat(30 - fieldName.length)
    message += `\n ${fieldName}: ${extraValueData+inputs[i].value};%0A`
  }
  console.log('message', message)
